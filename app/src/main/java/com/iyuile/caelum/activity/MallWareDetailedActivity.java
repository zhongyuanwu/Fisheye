 ...
 * 商城->商品->详细
 */
public class MallWareDetailedActivity extends BaseBackSwipeActivity implements View.OnClickListener {

    public static MallWareDetailedActivity mInstance;
    ...
    private long itemId, modelId;
    private int count = 1;

    private ErrorHandlingCallAdapter.MyCall<Void> callResponse;

    //----
    private Button btnShoppingCart, btnShoppingCartAdd;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        try {
            itemEntity = (ItemEntity) bundle.getSerializable(INTENT_PARAM_ENTITY);
            if (itemEntity == null) {
                finish();
                return;
            }
            itemId = itemEntity.getId();
            url = String.format(AppConstants.ITEM_URL_PARAM, itemId);
        } catch (Exception e) {
            finish();
            return;
        }
        ...
        setContentView(R.layout.activity_mall_ware_detailed);
...
//                initTopBarForLeftIcon(getString(R.string.title_web_loading), 0);
                initTopBarForLeftSecondaryIcon(getString(R.string.mall_ware), 0, 0, mApplication.getWoodBodyStyleFont());
                setStatusBar(this);
                initActionbar();
            } else {
...
//            initTopBarForLeftIcon(getString(R.string.title_web_loading), 0);
    ...
    private void initView() {
        btnShoppingCart = (Button) findViewById(R.id.wb_btn_shopping_cart);
        btnShoppingCartAdd = (Button) findViewById(R.id.wb_btn_shopping_cart_add);

        btnShoppingCart.setOnClickListener(this);
        btnShoppingCartAdd.setOnClickListener(this);
    ...
    public void onClick(View v) {
        if (ButtonUtils.isSeriesDoubleClick())
            return;
        switch (v.getId()) {
            case R.id.wb_btn_shopping_cart:
                startAnimActivity(ShoppingCartActivity.class);
                break;
            case R.id.wb_btn_shopping_cart_add:
                if (isStatus) return;
                //调用html页面的js方法获取modelID
                web.loadUrl("javascript:getModelId()");
                break;
        }
    }
    ...
    private void addShoppingCartApi(long itemId) {
        isStatus = true;
        callResponse = ApiServiceImpl.addShoppingCartImpl(this, itemId, modelId, count, new ResponseHandlerListener<Void>(this) {
            ...
            public void success(final int statusCode, final Response<Void> response) {
                ...
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        if (statusCode == NetworkConstants.RESPONSE_CODE_CREATED_201) {
                            SuperToast.makeText(MallWareDetailedActivity.this, getResources().getString(R.string.shoppingcart_prompt_add_complete),
                                    SuperToast.Icon.Resource.YES,
                                    SuperToast.Background.GREEN).show();
                        } else {
                            SuperToast.makeText(MallWareDetailedActivity.this, getResources().getString(R.string.shoppingcart_prompt_add_already_exist),
            ...
            public void clientError(final int statusCode, final Response<?> response) {
                ...
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        if (statusCode == NetworkConstants.RESPONSE_CODE_UNPROCESSABLE_ENTITY_422 || statusCode == NetworkConstants.RESPONSE_CODE_BAD_REQUEST_400) {
                            if (response.errorBody() != null) {
                                try {
                                    Converter<ResponseBody, ErrorResponse> errorConverter = RetrofitUtils.getRetrofit(MallWareDetailedActivity.this).
                                            responseBodyConverter(ErrorResponse.class, new Annotation[0]);
                                    ErrorResponse errorObject = errorConverter.convert(response.errorBody());

                                    if (errorObject.getErrors() != null) {
                                        if (errorObject.getErrors().getModel_id() != null) {
                                            SuperToast.makeText(MallWareDetailedActivity.this, errorObject.getErrors().getModel_id()[0],
                                    ...
                                    if (errorObject.getMessage() != null)
                                        SuperToast.makeText(MallWareDetailedActivity.this, errorObject.getMessage(),
                            ...
                            SuperToast.makeText(MallWareDetailedActivity.this, getString(R.string.shoppingcart_prompt_add_failure),
                                    SuperToast.Icon.Resource.ERROR,
                                    SuperToast.Background.RED).show();
                        }
                    }
...
//        } catch (NullPointerException e) { }
    ...
    private void webConfigure() {
        ...
        if (itemId != 0)
            web.addJavascriptInterface(new JavaScriptObject(), "caelum");// 设置本地调用对象及其接口
...
//                progressBar.setProgress(newProgress);
            ...
            public void onReceivedTitle(WebView view, String title) {
                ...
                if (MallWareDetailedActivity.this.title == null) {
...
//                Log.e(":::webview:onPageFinished", "url = " + url);
            ...
            private WebResourceResponse handleRequestViaOkHttp(@NonNull String url) {
                try {
                    OkHttpClient client = OkHttpUtils.getInstance(MallWareDetailedActivity.this);
    ...
    public Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case 1:
                    try {
                        modelId = (long) msg.obj;
                        for (ItemModelEntity itemModelEntity : itemEntity.getModels().getData()) {
                            if (modelId == itemModelEntity.getId()) {
                                addShoppingCartApi(itemId);
                                return;
                            }
                        }
                        isStatus = false;
                        modelId = 0;
                        SuperToast.makeText(MallWareDetailedActivity.this, getResources().getString(R.string.mall_ware_detailed_prompt_model_empty),
                                SuperToast.Icon.Resource.INFO,
                                SuperToast.Background.BLUE).show();
                    } catch (Exception e) {
                    }
    ...
    public void onPause() {
        super.onPause();
        MobclickAgent.onPageEnd(MallWareDetailedActivity.class.getSimpleName());
    ...
    public void onResume() {
        super.onResume();
        MobclickAgent.onPageStart(MallWareDetailedActivity.class.getSimpleName());
