...
/**
 * 检测手机晃动
 * {@links https://github.com/square/seismic}
...
public class ShakeDetector implements SensorEventListener {
    ...
    public void actualOnSensorChanged(final float[] values, long timestamp) {
        ...
        if (!queue.canDecide()) {
            ...
            // the device is shaking, and we've seen enough of a pause to be sure it's not just a continuation of the previous shake.
            queue.clear();
            listener.hearShake();
            stillShaking = true;
        } else {
...
//    private boolean isAccelerating(SensorEvent event) {
    ...
    static class SampleQueue {
         ...
         */
        boolean isShaking() {
            return canDecide() && acceleratingCount >= (sampleCount >> 1) + (sampleCount >> 2);
