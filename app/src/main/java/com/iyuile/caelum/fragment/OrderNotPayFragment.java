...
//        listView.setRefreshing(true);// 自动刷新数据自动执行{@link #onPullDownToRefresh(PullToRefreshBase)}
    ...
    private void loadingData(final int page) {
        ...
        callResponse = ApiServiceImpl.findOrderListImpl(getActivity(), page, OrderStatusValue.UNPAID, new ResponseHandlerListener<OrdersResponse>(getActivity()) {
...
//        } catch (NullPointerException e) {}
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == getActivity().RESULT_OK) {
            try {
                if (requestCode == OrderFragment.mInstance.REQUEST_CODE_RESULT_REFRESHING)
                    loadingData(1);
            } catch (Exception e) {
            }
        }
    ...
    public void onPause() {
        super.onPause();
        MobclickAgent.onPageEnd("Main_Order_NotPay");
    }

    @Override
    public void onResume() {
        super.onResume();
        MobclickAgent.onPageStart("Main_Order_NotPay");
    }
