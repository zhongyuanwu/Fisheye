...
//        listView.setRefreshing(true);// 自动刷新数据自动执行{@link #onPullDownToRefresh(PullToRefreshBase)}
    ...
    private void loadingData(final int page) {
        ...
        callResponse = ApiServiceImpl.findOrderListImpl(getActivity(), page, OrderStatusValue.FINISH, new ResponseHandlerListener<OrdersResponse>(getActivity()) {
...
//        } catch (NullPointerException e) {}
    ...
    public void onPause() {
        super.onPause();
        MobclickAgent.onPageEnd("Main_Order_AlreadyCompleted");
    }

    @Override
    public void onResume() {
        super.onResume();
        MobclickAgent.onPageStart("Main_Order_AlreadyCompleted");
    }
