 ...
 * @author WangYao
 */
public class OrderNotShippedFragment extends BaseLazyFragment implements AbsListView.OnScrollListener {
    @Bind(R.id.not_shipped_refresh)
    SwipeRefreshLayout notShippedRefresh;
    @Bind(R.id.not_shipped_recycler)
    EmptyRecyclerView notShippedRecycler;
    @Bind(R.id.not_shipped_empty_view)
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        if (null != rootView) {
            ...
            rootView = inflater.inflate(R.layout.fragment_order_not_shipped, null);
    ...
    protected void lazyLoad() {
        if (!isPrepared || !isVisible || isRequestData)
            return;
        Log.e(":::未发货", "加载数据");
        notShippedRefresh.post(new Runnable() {
            @Override
            public void run() {
                notShippedRefresh.setRefreshing(true);
                loadingData(1);
            }
        });

        notShippedRefresh.setOnRefreshListener(new SwipeRefreshLayout.OnRefreshListener() {
            @Override
    ...
    private void initView() {
        notShippedRefresh.setColorSchemeResources(R.color.colorPrimary);
        mAdapter = new OrderAllAdapter(_mActivity, listData);
        notShippedRecycler.setLayoutManager(new LinearLayoutManager(_mActivity));
        notShippedRecycler.addItemDecoration(new MyItemDecoration());
        notShippedRecycler.setEmptyView(mEmptyView);
        notShippedRecycler.setAdapter(mAdapter);
//        listView = (PullToRefreshListView) rootView.findViewById(R.id.list_view_order_not_shipped);
...
//        });
    ...
    private void loadingData(final int page) {
        ...
        callResponse = ApiServiceImpl.findOrderListImpl(getActivity(), page, OrderStatusValue.PAID, new ResponseHandlerListener<OrdersResponse>(getActivity()) {
...
//                                List<NewsEntity> listDatas = Arrays.asList(newsResponse.getData());
            ...
            public void onFinish() {
                super.onFinish();
                notShippedRefresh.setRefreshing(false);
            }
...
//        } catch (NullPointerException e) {}
    ...
    public void onPause() {
        super.onPause();
        MobclickAgent.onPageEnd("Main_Order_NotShipped");
    }

    @Override
    public void onResume() {
        super.onResume();
        MobclickAgent.onPageStart("Main_Order_NotShipped");
    }
