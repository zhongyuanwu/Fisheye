 ...
 * @author WangYao
 */
public class OrderAlreadyShippedFragment extends BaseLazyFragment implements AbsListView.OnScrollListener {

    @Bind(R.id.already_shipped_refresh)
    SwipeRefreshLayout alreadyShippedRefresh;
    @Bind(R.id.already_shipped_recycler)
    EmptyRecyclerView alreadyShippedRecycler;
    @Bind(R.id.already_shipped_empty_view)
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        if (null != rootView) {
            ...
            rootView = inflater.inflate(R.layout.fragment_order_already_shipped, null);
    ...
    protected void lazyLoad() {
        if (!isPrepared || !isVisible || isRequestData)
            return;
        Log.e(":::已发货", "加载数据");
        alreadyShippedRefresh.post(new Runnable() {
            @Override
            public void run() {
                alreadyShippedRefresh.setRefreshing(true);
                loadingData(1);
            }
        });

        alreadyShippedRefresh.setOnRefreshListener(new SwipeRefreshLayout.OnRefreshListener() {
            @Override
    ...
    private void initView() {
        alreadyShippedRefresh.setColorSchemeResources(R.color.colorPrimary);
        mAdapter = new OrderAllAdapter(_mActivity, listData);
        alreadyShippedRecycler.setLayoutManager(new LinearLayoutManager(_mActivity));
        alreadyShippedRecycler.addItemDecoration(new MyItemDecoration());
        alreadyShippedRecycler.setEmptyView(mEmptyView);
        alreadyShippedRecycler.setAdapter(mAdapter);
//        listView = (PullToRefreshListView) rootView.findViewById(R.id.list_view_order_already_shipped);
...
//        });
    ...
    private void loadingData(final int page) {
        ...
        callResponse = ApiServiceImpl.findOrderListImpl(getActivity(), page, OrderStatusValue.SEND, new ResponseHandlerListener<OrdersResponse>(getActivity()) {
...
//                                List<NewsEntity> listDatas = Arrays.asList(newsResponse.getData());
            ...
            public void onFinish() {
                super.onFinish();
                alreadyShippedRefresh.setRefreshing(false);
            }
...
//        } catch (NullPointerException e) {}
    ...
    public void onPause() {
        super.onPause();
        MobclickAgent.onPageEnd("Main_Order_AlreadyShipped");
    }

    @Override
    public void onResume() {
        super.onResume();
        MobclickAgent.onPageStart("Main_Order_AlreadyShipped");
    }
